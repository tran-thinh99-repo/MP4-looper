...
def upload_files(output_folder, only_file=None, progress_callback=None, status_callback=None):
    """
    Simple function to upload files to Google Drive.
    This can be called directly from the main application.
    
    Args:
        output_folder: Folder containing files to upload
        only_file: Only upload this specific file (optional)
    ...
    Returns:
        True if upload started successfully, False otherwise
    """
    try:
        # Create uploader
        ...
        # Set callbacks
        uploader.set_callbacks(progress_callback, status_callback)
        ...
        # Add files to queue
        if not uploader.upload_folder(output_folder, only_file):
            return False
...
def main():
    ...
    parser.add_argument("--file", help="Only upload this specific file")
    ...
    # Set up simple console progress reporting
    def report_progress(filename, progress):
        percent = int(progress * 100)
        print(f"\rUploading {filename}: {percent}% complete", end="", flush=True)
        if progress >= 1.0:
            print()  # New line after completion
    
    uploader.set_callbacks(progress_callback=report_progress, status_callback=print)
    ...
    # Handle retry mode
    if args.retry:
        if not uploader.retry_failed(args.folder):
            print("No failed uploads to retry or retry failed")
            return 1
        print("Retrying failed uploads...")
    else:
        # Add files to queue
        if not uploader.upload_folder(args.folder, args.file):
            print("Failed to add files to upload queue")
            return 1
    ...
    print("Upload started in background. Press Ctrl+C to stop.")
    try:
        while uploader.is_uploading:
            time.sleep(1)
        
        if uploader.failed_uploads:
            print(f"Upload completed with {len(uploader.failed_uploads)} failures")
            return 1
        else:
